...
const { set, select } = require("../../../config/redis");
...
const setKingIsAttacked =(isAttacked) => {
    ...
    if(isAttacked){
        ...
        displayToast("Your king is attacked");
    }else{
...
const canMakeMove =({currentBox, boxToMove}, {piece, pieceToRemove, pieceToRemovePieceImg}) => {
    ...
    if(moveIsNotValid){
        ...
        displayToast("You cant move there, your king will be attacked")
...
const performCastling = (currentPlayer, rookPosition, kingPosition) => {
    ...
    if(currentPlayer === player){
        ...
        if(check){
            ...
            displayToast("Your king will be attacked");
        }else{
            if(rookPosition[0] === 'A'){
                isLeftCastlingPerformed = true;
            }else{
...
const checkForElPassant = (enemyMove) => {
    ...
    if(player === 'light'){
        enemyPawn = blackPieces.find(enemyPiece => enemyPiece.pieceToPromote === 'pawn' && enemyPiece.position === from);
    }else{
        enemyPawn = lightPieces.find(enemyPiece => enemyPiece.pieceToPromote === 'pawn' && enemyPiece.position === from);
    }
...
const performElPassant =(currentPlayer, prevPawnPosition, newPawnPosition) => {
    ...
    let capturedPawnBox = document.getElementById(capturePawnPosition);
...
const checkForDraw = () =>{
 ...
 return myTotalPieces === enemyTotalPieces === 1;
}
...
socket.on("receive-game-details", (details) => {
    ...
    if(gameHasTimer){
        timer = new Timer(player, roomId, gameDetails.time, 0, updateTimer, timerEndCallback);
    }
...
// If we are 1st player and someone joints than this event is emitted

socket.on("game-started", (playerTwo) => {
    gameStartedAtTimestamp = new Date().toISOString().slice(0, 19).replace('T', ' ');
    startGame(playerTwo); // Add a semicolon here
    if(gameHasTimer){
        timer.start();
    }
});

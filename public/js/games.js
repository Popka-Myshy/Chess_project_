...
const noGameMeessage = document.getElementById('no-game-message');
...
const joinRoonForm = document.getElementById('join-room-form');
    ...
    rooms.forEach(room => {
        ...
        gameList.innerHTML+= `
            <li class="game" id='${room.id}' data-withpassword='${hasPassword}'>
                <div class="user">
                    <span>${username}</span>
                    <span>( ${user_rank.charArt(0).toUpperCase() + user_rank.slice(1) }</span>
...
socket.on('receive-rooms', (rooms) => {
    if(rooms.length > 0){
        noGameMeessage.classList.add('hidden'); 
        gamesList.classList.remove('hidden');

        displayRooms(rooms);
    }else{
        gamesList.classList.add('hidden');
        noGameMeessage.classList.remove('hidden'); 
    }
});
